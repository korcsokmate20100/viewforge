...
// Kimenet: { hook, sections: [{label, content}], outro }
...
Videó ötlet/téma: ${topic}

Írj egy konkrét, forgatásra kész vázlatot ehhez a videóhoz:
- A hook legyen szó szerint elmondható mondat vagy pontos szituáció-leírás, ami az első pillanatokban megragadja a nézőt
- 3-5 fő szakaszra bontsd a videót, mindegyikhez adj konkrét instrukciót (mit mutass, mit mondj, milyen csavar jöjjön)
...
Válaszolj KIZÁRÓLAG egy valid JSON objektummal, semmi mást:
{
  ...
  "sections": [
    {"label": "szakasz neve", "content": "konkrét instrukció, mit csinálj/mondj"}
  ],
  "outro": "záró mondat és CTA"
}`;
    ...
    let script;
    try { script = JSON.parse(text.replace(/```json|```/g, '').trim()); }
    ...
    return new Response(JSON.stringify(script), { status: 200, headers: CORS });
  } catch (err) {
    console.error(err);
